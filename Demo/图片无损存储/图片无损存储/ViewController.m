...
//// 用户点击 不允许访问
//UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"用户不允许用户开启相册权限" message:nil delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
//[alertView show];
...
- (void)viewDidLoad {
    ...
    UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:self.imageView.bounds byRoundingCorners:UIRectCornerTopRight | UIRectCornerTopLeft | UIRectCornerBottomLeft | UIRectCornerBottomRight cornerRadii:CGSizeMake(10, 10)];
    ...
    if (status == PHAuthorizationStatusAuthorized) {
        ...
        [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status) {
            if(status == PHAuthorizationStatusAuthorized) {
                ...
                // 用户点击 不允许访问
                self.labelInfo.text = @"失败，请开启相机权限";
            }
        }];
    }
}
...
//    NSString *myAlbumName = [NSBundle mainBundle].infoDictionary[@"CFBundleDisplayName"];
    // 也可以直接自定义
    NSString *myAlbumName = @"albumName";
